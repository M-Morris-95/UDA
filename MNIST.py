...
test = np.reshape(test.values, (-1,28,28))
...
def create_log_dir():
    ...
    log_dir =str(sys.argv[0][:-9]) + "/" + FLAGS.log_dir + "_" + timestamp
    if not os.path.exists(log_dir):
        os.makedirs(log_dir)

    # save command line arguments
    with open(log_dir + "/hyperparameters"+".csv", "wb") as f:
        for arg in FLAGS.__dict__:
            f.write(bytearray((arg + "," + str(FLAGS.__dict__[arg]) + "\n"), 'utf8'))
...
if FLAGS.train:
    ...
    assert os.path.exists(FLAGS.img_path), "The specified path to the images does not exist: {}". \
